...
<script type="text/javascript">
	...
	function keywordSearch() {
		...
		$.ajax({
			...
			success : function(data, textStatus) {
				var jsonInfo = JSON.parse(data); // 전송된 데이터를 JSON으로 파싱함
				displayResult(jsonInfo); // 전송된 JSON 데이터를 표시함
			},
	...
	function displayResult(jsonInfo) {
		var count = jsonInfo.keyword.length; // json의 데이터 갯수를 구함
		if (count > 0) {
			var html = '';
			
			// json데이터를 차례대로 <a>태그를 이용해 키워드 목록을 만듬
			for (var i in jsonInfo.keyword) {
				html += "<a href=\"javascript:select('" + jsonInfo.keyword[i]
						+ "')\">" + jsonInfo.keyword[i] + "</a><br/>";
			}
			// <a>태그로 만든 키워드 목록을 <div>태그에 차례대로 표시함
