				...
				<c:otherwise>
					<tr>
						<form name="frm_order_all_cart">
							<!-- 장바구니에 등록된 상품번호로 조회한 상품 목록을 표시함 -->
							<c:forEach var="item" items="${myGoodsList }" varStatus="cnt">
							
								<!-- 장바구니에 담긴 상품 수량을 표시하기 위해 변수를 설정함 -->
								<c:set var="cart_goods_qty" value="${myCartList[cnt.count-1].cart_goods_qty}" />
								<c:set var="cart_id" value="${myCartList[cnt.count-1].cart_id}" />
								<td>
									<input type="checkbox" name="checked_goods" 
											checked value="${item.goods_id }"
											onClick="calcGoodsPrice(${item.goods_sales_price },this)">
								</td>
								<td class="goods_image">
									<a href="${contextPath}/goods/goodsDetail.do?goods_id=${item.goods_id }">
										<!-- 상품의 이미지를 표시함 -->
										<img width="75" alt="" src="${contextPath}/thumbnails.do?
												goods_id=${item.goods_id}&fileName=${item.goods_fileName}" />
									</a>
								</td>
								<td>
									<h2>
										<!-- 상품 이름을 표시함 -->
										<a href="${contextPath}/goods/goodsDetail.do?
												goods_id=${item.goods_id }">${item.goods_title }
										</a>
									</h2>
								</td>
	...
	<table width=80% class="list_view" style="background: #cacaff">
		<tbody>
			...
			<tr cellpadding=40 align=center>
				...
				<td>
					<img width="25" alt="" src="${contextPath}/resources/image/minus.jpg"></td>
				<td>
